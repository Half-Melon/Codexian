...
Review this code: $ARGUMENTS`;
      ...
      expect(parsed.model).toBe('codex-gpt-5.4-mini-4-5');
...
Prompt content`;
    ...
    it('should handle block scalar at end of frontmatter', () => {
      const content = `---
model: codex-gpt-5.3-codex-spark-4-5
description: |
...
Prompt`;
      ...
      expect(parsed.model).toBe('codex-gpt-5.3-codex-spark-4-5');
    });
...
describe('serializeSlashCommandMarkdown', () => {
  it('serializes all fields in kebab-case', () => {
    ...
    expect(result).toContain('model: codex-gpt-5.4-mini-4-5');
