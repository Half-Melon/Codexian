...
describe('CodexSettingsTab', () => {
  ...
  it('reconciles the Codex saved model when a removed custom model was selected', async () => {
    Object.defineProperty(process, 'platform', { value: 'darwin' });
    const plugin = createPlugin({
      settingsProvider: 'codex',
    ...
    expect(plugin.settings.model).toBe('gpt-5.4-mini');
    expect(plugin.settings.savedProviderModel).toEqual({ codex: 'my-custom-model' });
    expect(mockSaveSettings).toHaveBeenCalledTimes(1);
  });
});
