...
describe('TabManager - Persistence', () => {
    ...
    it('should persist draftModel for blank tabs', async () => {
      const blankManager = createManager({
        tabFactory: () => createMockTabData({
          ...
          draftModel: 'codex:google/gemini-3.1-pro-preview',
          providerId: 'codex',
        }),
      });
  ...
  describe('restoreState', () => {
    ...
    it('only primes the final active restored pre-session Codex tab', async () => {
      const runtimeCommandLoader = {
        isAvailable: jest.fn().mockReturnValue(true),
        loadCommands: jest.fn().mockResolvedValue([{ id: 'codex:review', name: 'review', content: '' }]),
      };
      const mockCatalog = {
      ...
      mockGetCapabilities.mockImplementation((providerId: string) => ({
        ...
        supportsPersistentRuntime: true,
        supportsNativeHistory: true,
        supportsPlanMode: providerId === 'codex',
        supportsRewind: providerId === 'codex',
        supportsFork: providerId === 'codex',
        supportsProviderCommands: true,
        reasoningControl: providerId === 'codex' ? 'effort' : 'none',
      }));

      const plugin = createMockPlugin({
        getConversationById: jest.fn().mockImplementation(async (conversationId: string) => {
          if (conversationId === 'conv-active') {
            return {
              id: 'conv-active',
              messages: [{ id: 'm1' }],
              providerState: {},
      ...
      const manager = createManager({
        plugin,
        tabFactory: (n) => createMockTabData({
          id: n === 1 ? 'restored-inactive' : 'restored-active',
          providerId: 'codex',
          conversationId: n === 1 ? 'conv-inactive' : 'conv-active',
      ...
      await manager.restoreState({
        openTabs: [
          { tabId: 'restored-inactive', conversationId: 'conv-inactive' },
          { tabId: 'restored-active', conversationId: 'conv-active' },
        ],
        activeTabId: 'restored-active',
      });
      await flushMicrotasks();

      expect(manager.getActiveTabId()).toBe('restored-active');
      expect(runtimeCommandLoader.loadCommands).toHaveBeenCalledTimes(1);
      expect(runtimeCommandLoader.loadCommands).toHaveBeenCalledWith(expect.objectContaining({
        conversation: expect.objectContaining({ id: 'conv-active' }),
      }));
      expect(mockCatalog.setRuntimeCommands).toHaveBeenCalledTimes(1);
    });
  });
});
...
describe('TabManager - Broadcast', () => {
  ...
  describe('broadcastToProviderTabs', () => {
    it('should call every initialized Codex runtime', async () => {
      manager = createManager({
        tabFactory: (n) => createMockTabData({
          id: `tab-${n}`,
          providerId: 'codex',
      ...
      const broadcastFn = jest.fn().mockResolvedValue(undefined);
      await manager.broadcastToProviderTabs('codex', broadcastFn);

      expect(broadcastFn).toHaveBeenCalledTimes(2);
      expect(broadcastFn).toHaveBeenCalledWith(expect.objectContaining({ providerId: 'codex' }));
    });
  });
});
...
describe('TabManager - SDK Commands', () => {
  ...
  it('should reuse commands from another ready tab with the same provider', async () => {
    ...
    const manager = createManager({
      tabFactory: (n) => createMockTabData({
        id: `tab-${n}`,
        providerId: 'codex',
        service: n === 1 ? readyCodexService : null,
      }),
    });

    await manager.createTab();
    const lazyCodexTab = await manager.createTab();

    await expect(manager.getSdkCommands(lazyCodexTab!.id)).resolves.toEqual(supportedCommands);
    expect(readyCodexService.getSupportedCommands).toHaveBeenCalledTimes(1);
  });

  it('should reuse commands from another ready Codex tab', async () => {
    const codexCommands = [{ id: 'sdk:commit', name: 'commit', content: '' }];
    const readyCodexService = {
      providerId: 'codex',
      isReady: jest.fn().mockReturnValue(true),
      getSupportedCommands: jest.fn().mockResolvedValue(codexCommands),
    };
    const manager = createManager({
      tabFactory: (n) => createMockTabData({
        id: `tab-${n}`,
    ...
    await expect(manager.getSdkCommands(codexTab!.id)).resolves.toEqual(codexCommands);
    expect(readyCodexService.getSupportedCommands).toHaveBeenCalledTimes(1);
  });

  it('should resolve blank-tab SDK commands through the Codex provider', async () => {
    const codexCommands = [{ id: 'sdk:commit', name: 'commit', content: '' }];
    const readyCodexService = {
      providerId: 'codex',
      isReady: jest.fn().mockReturnValue(true),
    ...
    await expect(manager.getSdkCommands(blankCodexTab!.id)).resolves.toEqual(codexCommands);
    expect(readyCodexService.getSupportedCommands).toHaveBeenCalledTimes(1);
  });

  it('should keep inactive blank Codex tabs cold when SDK commands are requested', async () => {
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    const runtimeCommandLoader = {
    ...
    const tab = await manager.createTab(undefined, 'tab-codex', { activate: false });
  ...
  it('should invalidate cached Codex commands when the saved session context changes', async () => {
    const firstCommands = [{ id: 'codex:review', name: 'review', content: '' }];
    const secondCommands = [{ id: 'codex:compact', name: 'compact', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    const runtimeCommandLoader = {
    ...
    const plugin = createMockPlugin({
      getConversationById: jest.fn()
        .mockResolvedValueOnce({
          id: 'conv-codex',
          messages: [{ id: 'm1' }],
          providerState: { databasePath: '/persisted/codex.db' },
          sessionId: 'session-1',
        })
        .mockResolvedValueOnce({
          id: 'conv-codex',
    ...
    runtimeCommandLoader.loadCommands.mockClear();
    mockCatalog.setRuntimeCommands.mockClear();
    const tab = await manager.createTab('conv-codex', 'tab-codex', { activate: false });
  ...
  it('should prime active blank Codex tabs automatically', async () => {
    const supportedCommands = [{ id: 'codex:review', name: 'review', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    const runtimeCommandLoader = {
  ...
  it('should prime the active restored Codex conversation tab automatically', async () => {
    const supportedCommands = [{ id: 'codex:review', name: 'review', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    ...
    await manager.createTab('conv-codex', 'tab-codex-restored', { activate: false });
    await flushMicrotasks();
  ...
  it('should prime the active restored pre-session Codex conversation tab automatically', async () => {
    const supportedCommands = [{ id: 'codex:review', name: 'review', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    ...
    await manager.createTab('conv-codex', 'tab-codex-pre-session', { activate: false });
    await flushMicrotasks();
  ...
  it('should prime restored Codex conversation tabs when they become active', async () => {
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    const runtimeCommandLoader = {
    ...
    const manager = createManager({
      plugin,
      tabFactory: (n) => createMockTabData({
        id: n === 1 ? 'tab-codex' : 'tab-codex-restored',
        providerId: 'codex',
        conversationId: 'conv-codex',
    ...
    await manager.createTab('conv-codex', 'tab-codex');
    runtimeCommandLoader.loadCommands.mockClear();
    mockCatalog.setRuntimeCommands.mockClear();
  ...
  it('should not borrow ready Codex commands from another tab session', async () => {
    const readyCommands = [{ id: 'codex:review', name: 'review', content: '' }];
    const loaderCommands = [{ id: 'codex:compact', name: 'compact', content: '' }];
    const readyService = {
      providerId: 'codex',
      isReady: jest.fn().mockReturnValue(true),
    ...
    const manager = createManager({
      ...
      tabFactory: (n) => createMockTabData({
        id: `tab-${n}`,
        providerId: 'codex',
        conversationId: n === 2 ? 'conv-codex' : null,
    ...
    const coldTab = await manager.createTab('conv-codex');
  ...
  it('should keep an active restored Codex conversation tab cold', async () => {
    ProviderWorkspaceRegistry.setServices('codex', {
      commandCatalog: {
        setRuntimeCommands: jest.fn(),
      } as any,
    ...
    mockGetCapabilities.mockImplementation((providerId: string) => ({
      ...
      supportsRewind: true,
      supportsFork: true,
      supportsProviderCommands: providerId === 'codex',
      reasoningControl: 'effort',
    }));
    const plugin = createMockPlugin({
      getConversationById: jest.fn().mockResolvedValue({
    ...
    const tab = await manager.createTab('conv-codex', 'tab-codex-restored', { activate: false });
    await flushMicrotasks();
...
describe('TabManager - Provider Command Catalog', () => {
  ...
  it('should resolve the blank-tab catalog from draftModel instead of stale providerId', async () => {
    const codexCatalog = {
      listDropdownEntries: jest.fn().mockResolvedValue([
        {
          id: 'codex-command-test', providerId: 'codex', kind: 'command',
          name: 'codex-only', description: 'Codex command', content: '',
          scope: 'vault', source: 'user', isEditable: true, isDeletable: true,
          displayPrefix: '/', insertPrefix: '/',
        },
      ]),
  ...
  it('should refresh Codex runtime commands before listing catalog entries', async () => {
    ...
    expect(readyService.getSupportedCommands).toHaveBeenCalledTimes(1);
    expect(codexCatalog.setRuntimeCommands).toHaveBeenCalledWith(supportedCommands);
    expect(codexCatalog.listDropdownEntries).toHaveBeenCalledWith({ includeBuiltIns: false });
  });

  it('should clear Codex runtime commands when revalidation returns no commands', async () => {
    const readyService = {
      providerId: 'codex',
      isReady: jest.fn().mockReturnValue(true),
    ...
    await expect(manager.getSdkCommands(tab!.id)).resolves.toEqual([]);
    expect(codexCatalog.setRuntimeCommands).toHaveBeenCalledWith([]);
  });
  ...
  it('starts blank-tab provider warmup in the background from the provider-change callback', async () => {
    ...
    let settled = false;
    const callbackPromise = Promise.resolve(options.onProviderChanged('codex')).then(() => {
      settled = true;
    });
  ...
  it('should return null catalog config when provider has no catalog', async () => {
    // No catalog assigned to registry for 'codex'
...
describe('TabManager - Callback Wiring', () => {
  ...
  describe('ChatState callbacks during tab creation', () => {
    it('should wire onStreamingChanged callback to TabManager callbacks', async () => {
      ...
      const manager = new TabManager(createMockPlugin(), createMockEl(), createMockView(), callbacks);
      await manager.createTab();
...
describe('TabManager - forkToNewTab', () => {
  it('should propagate currentNote from context to forked conversation', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-1', providerId: 'codex' });
  ...
  it('should not set currentNote when context has none', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-2', providerId: 'codex' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
...
describe('TabManager - forkInCurrentTab', () => {
  ...
  it('should return false when no active tab exists', async () => {
    const plugin = createMockPlugin({
      createConversation: jest.fn().mockResolvedValue({ id: 'fork-conv-2', providerId: 'codex' }),
  ...
  it('should not check tab count limit', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-3', providerId: 'codex' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
    const mockSwitchTo = jest.fn().mockResolvedValue(undefined);
...
describe('TabManager - forkToNewTab at max tabs', () => {
  it('should return null when at max tabs', async () => {
    ...
    plugin.createConversation = jest.fn().mockResolvedValue({ id: 'fork-conv', providerId: 'codex' });
...
describe('TabManager - buildForkTitle', () => {
  function setupTitleTest(existingTitles: string[] = []) {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv', providerId: 'codex' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
