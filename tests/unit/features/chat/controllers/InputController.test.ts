...
function createMockAgentService() {
  return {
    ...
    prepareTurn: jest.fn().mockImplementation((request: any) => encodeCodexTurn(request)),
    query: jest.fn(),
...
function createMockDeps(overrides: Partial<InputControllerDeps> = {}): InputControllerDeps & { mockAgentService: ReturnType<typeof createMockAgentService> } {
  ...
  return {
    ...
    getSubagentStateTracker: () => ({ resetSpawnedCount: jest.fn(), resetStreamingState: jest.fn() }) as any,
    mockAgentService,
    ...overrides,
  };
}
...
describe('InputController - Message Queue', () => {
  ...
  describe('Queue indicator UI', () => {
    it('should show queue indicator when message is queued', () => {
      ...
      expect(queueIndicatorEl.querySelector('.codexidian-queue-indicator-text')?.textContent).toBe('⌙ Queued: test message');
    ...
    it('should truncate long message preview in indicator', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      const text = queueIndicatorEl.querySelector('.codexidian-queue-indicator-text')?.textContent as string;
    ...
    it('should show [images] when queue message has only images', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      expect(queueIndicatorEl.querySelector('.codexidian-queue-indicator-text')?.textContent).toBe('⌙ Queued: [images]');
    });
    ...
    it('should show Codex steer action when queued message can be steered', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      expect(queueIndicatorEl.querySelector('.codexidian-queue-indicator-action')?.textContent).toBe('Steer Now');
    });
    ...
    it('should steer the queued Codex message when the action is clicked', async () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      queueIndicatorEl.querySelector('.codexidian-queue-indicator-action')?.click();
      await Promise.resolve();
      await Promise.resolve();
      ...
      expect(deps.state.queuedMessage).toBeNull();
      expect(queueIndicatorEl.querySelector('.codexidian-queue-indicator-text')?.textContent)
        .toBe('⌙ Steering: queued follow-up');
      expect(queueIndicatorEl.querySelector('.codexidian-queue-indicator-action')).toBeNull();
      expect(queueIndicatorEl.style.display).toBe('flex');
  ...
  describe('Sending messages', () => {
    ...
    it('should persist replay-safe user content instead of transport-only prompt', async () => {
      ...
      (deps as any).mockAgentService.prepareTurn = jest.fn().mockReturnValue({
        request: { text: 'Review this note' },
        persistedContent: 'Review this note',
        prompt: 'Review this note\n\n[Current note: notes/review.md]',
        isCompact: false,
      });
      (deps as any).mockAgentService.query = jest.fn().mockImplementation(() => createMockStream([{ type: 'done' }]));

      inputEl = deps.getInputEl() as ReturnType<typeof createMockInputEl>;
      inputEl.value = 'Review this note';
      controller = new InputController(deps);

      await controller.sendMessage();

      expect(deps.state.messages[0].content).toBe('Review this note');
      expect(deps.state.messages[0].content).not.toContain('[Current note');
    });
    ...
    it('should prepend current note only once per session', async () => {
      ...
      expect(prompts[0]).toContain('[Current note: notes/session.md]');
      expect(prompts[1]).not.toContain('[Current note:');
    });
  ...
  describe('Editor context', () => {
    ...
    it('should preserve preview selection text without fabricating line attributes', async () => {
      ...
      const queryCall = ((deps as any).mockAgentService.query as jest.Mock).mock.calls[0];
      const promptSent = queryCall[0].prompt;
      expect(promptSent).toContain('[Editor selection from test/note.md:\n  selected text\nsecond line  \n]');
  ...
  describe('handleApprovalRequest', () => {
    ...
    it.each([
      ...
      const items = parentEl.querySelectorAll('codexidian-ask-item');
      const target = items.find((item: any) => {
        const label = item.querySelector('codexidian-ask-item-label');
    ...
    it('should render header metadata when approvalOptions provided', async () => {
      ...
      const reasonEl = parentEl.querySelector('codexidian-ask-approval-reason');
      expect(reasonEl?.textContent).toBe('Command is destructive');

      const pathEl = parentEl.querySelector('codexidian-ask-approval-blocked-path');
      expect(pathEl?.textContent).toBe('/usr/bin/rm');

      const agentEl = parentEl.querySelector('codexidian-ask-approval-agent');
    ...
    it('should render provider-supplied approval options and network-specific context', async () => {
      ...
      const descEl = parentEl.querySelector('codexidian-ask-approval-desc');
      expect(descEl?.textContent).toContain('api.openai.com');

      const items = parentEl.querySelectorAll('codexidian-ask-item');
      const labels = items
        .map((item: any) => item.querySelector('codexidian-ask-item-label')?.textContent)
        .filter(Boolean);
    ...
    it.each([
      ...
      async (optionLabel, optionValue, expectedDecision) => {
        ...
        const approvalPromise = controller.handleApprovalRequest(
          ...
          'Codex wants to access a path outside the working directory.',
          {
            decisionOptions: [
  ...
  describe('handleInstructionSubmit', () => {
    ...
    it('should pass the active chat model into instruction refine service', async () => {
      ...
      deps = createMockDeps({
        getAuxiliaryModel: () => 'codex:openai/gpt-5.4',
        getInstructionRefineService: () => mockInstructionRefineService as any,
      });
      deps.plugin.settings.systemPrompt = '';
