...
describe('StreamController - Text Content', () => {
  ...
  describe('Usage handling - edge cases', () => {
    it('should skip usage when subagentsSpawnedThisStream > 0', async () => {
      const msg = createTestMessage();
      (deps.subagentStateTracker as any).subagentsSpawnedThisStream = 1;
  ...
  describe('Codex subagent lifecycle', () => {
    it('renders prompt immediately and final result after wait_agent resolves', async () => {
      ...
      expect(deps.subagentStateTracker.recordSpawn).toHaveBeenCalledWith('spawn-1');
      ...
      expect(deps.subagentStateTracker.markFinished).toHaveBeenCalledWith('spawn-1');
    });
  });

  describe('Structured tool result handling', () => {
