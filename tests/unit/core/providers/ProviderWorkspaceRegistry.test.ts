...
describe('ProviderWorkspaceRegistry', () => {
  ...
  it('returns the assigned catalog for a provider', () => {
    ...
    expect(ProviderWorkspaceRegistry.getCommandCatalog('codex')).toBe(mockCatalog);
  });
  ...
  it('returns the runtime command loader for a provider', () => {
    ...
    expect(ProviderWorkspaceRegistry.getRuntimeCommandLoader('codex')).toBe(runtimeCommandLoader);
  });
  ...
  it('returns the tab warmup policy for a provider', () => {
    ...
    expect(ProviderWorkspaceRegistry.getTabWarmupPolicy('codex')).toBe(tabWarmupPolicy);
  });
});
