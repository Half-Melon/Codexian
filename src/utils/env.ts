...
function getExtraBinaryPaths(): string[] {
  ...
  if (isWindows) {
    ...
    if (home) {
      ...
      paths.push(path.join(home, '.codex', 'bin'));
...
export function getMissingNodeError(cliPath: string, enhancedPath?: string): string | null {
  ...
  return 'Codex CLI requires Node.js, but Node was not found on PATH. Install Node.js or use a native Codex binary, then restart Obsidian.';
}
