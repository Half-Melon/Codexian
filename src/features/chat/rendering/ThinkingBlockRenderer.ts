...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'codexidian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'codexidian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'codexidian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'codexidian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.codexidian-thinking-header');
  if (header) {
