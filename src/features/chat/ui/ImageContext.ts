...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.codexidian-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'codexidian-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.codexidian-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'codexidian-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'codexidian-drop-content' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.codexidian-input-wrapper');
    if (!inputWrapper) {
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'codexidian-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'codexidian-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'codexidian-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'codexidian-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'codexidian-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'codexidian-image-remove' });
    removeEl.setText('\u00D7');
