...
import { createDetachedSvg } from '../utils/dom';
...
export function createProviderIconSvg(
  ...
  if (icon.kind === 'markup') {
    ...
    const parsed = parser.parseFromString(
      `<svg xmlns="${NS}" viewBox="${icon.viewBox}">${icon.markup.trim()}</svg>`,
      'image/svg+xml',
    );
  ...
  const path = createDetachedSvg('path');
...
export function appendMcpIcon(el: HTMLElement | SVGElement): void {
  el.empty();
  const svg = createDetachedSvg('svg');
  ...
  const title = createDetachedSvg('title');
  title.setText('MCP');
...
export function setCodexianIcon(el: HTMLElement, icon: string): void {
  setIcon(el, icon);
}
